...
object BasicRepository : OldRepository {
    ...
    private fun <T> fetchData(path: String, parser: (String) -> T): T? {
        try {
            val url = Uri.parse(fullUrlString(path)).toString()

            Log.i(TAG, "fetching path $path")
            ...
            Log.e(TAG, "Error parsing path: $path ::: ${e.localizedMessage}")
        }
        return null
    }
    ...
    private fun parseGists(jsonString: String): List<Gist> {
        ...
        for (i in 0 until gistsArray.length()) {
            ...
            val gist = Gist("$i", Date(), gistObject.getString("description"), mapOf())
            gists.add(gist)
        }
